...
class RoadmapStreamService:
    ...
    async def stream_roadmap(
        request: GenerateRoadmapRequest, user_id: str | None
        ...
        try:
            ...
            async for event in roadmap_manager.stream_events(
                ...
                if event_type == "on_chain_end" and event_name == "plan_skeleton":
                    sse_event = RoadmapStreamService._handle_skeleton(event)
                    if sse_event:
                        yield sse_event

                # Handle actions generation
                elif event_type == "on_chain_end" and event_name == "generate_actions":
                    async for sse_event in RoadmapStreamService._handle_actions(event):
                        yield sse_event
                    ...
                    async for sse_event in RoadmapStreamService._handle_direct_actions(
    ...
    @staticmethod
    def _handle_skeleton(event: dict) -> str | None:
    ...
    @staticmethod
    async def _handle_actions(event: dict) -> AsyncGenerator[str, None]:
        ...
        for ms in milestones:
            ...
            if actions:
                ...
                evt = RoadmapActionsEvent(milestone_id=ms.id, actions=actions_view)
    ...
    async def _persist_roadmap(
        ...
        try:
            ...
            async with async_session_factory() as session:
                ...
                if len(milestones) > 0:
                    logger.info(
                        f"[Service] First Milestone keys: {milestones[0].keys()}"
                    )
                if len(goal_actions) > 0:
                    logger.info(
                        f"[Service] First Action keys: {goal_actions[0].keys()}"
                    )

                # Check if roadmap exists for this conversation
                existing_roadmap = None
                if request.conversation_id:
                    existing_roadmap = await repo.get_by_conversation_id(
                ...
                if existing_roadmap:
                    ...
                    roadmap = await repo.update_with_nodes(
